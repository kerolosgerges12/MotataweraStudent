...
public class ChapterAdapter extends RecyclerView.Adapter<ChapterAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        if (model.getPdf_url().isEmpty()) {
            holder.binding.chapterItemPDF.setText("NO PDF Available");

        } else {
            holder.binding.chapterItemPDF.setText("PDF Available");

        }
        if (model.getVideo_url().isEmpty()) {
            holder.binding.chapterItemVideo.setText("NO Video Available");

        } else {
            holder.binding.chapterItemVideo.setText("Video Available");
        ...
        holder.binding.layoutBtn.setOnClickListener(v -> {
            ...
            if (!model.getPdf_url().isEmpty() && !model.getVideo_url().isEmpty()) {
                ...
                Toast.makeText(context, "No Data Found Check Soon....", Toast.LENGTH_LONG).show();
    ...
    private void checklistempty(List<ChapterModel> list) {
        if (list.isEmpty()||list.size()==0){
            new androidx.appcompat.app.AlertDialog.Builder(context)
                    .setMessage("No Data Added till  now check soooon  ")
